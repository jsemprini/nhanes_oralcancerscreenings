...
gen examoc_t=.
replace examoc_t=0 if ohq880==2
replace examoc_t=1 if ohq880==1

gen examoc_n=.
replace examoc_n=0 if ohq885==2
replace examoc_n=1 if ohq885==1

tab ohq880 ohq895, missing

gen currentexam=.
replace currentexam=0 if ohq895>1 & ohq895<=3
replace currentexam=0 if ohq880==2
replace currentexam=1 if ohq895==1

gen recentexam=. 
replace recentexam=0 if ohq895==3 | ohq880==2
replace recentexam=1 if ohq895==1 | ohq895==2

tab ohq880 currentexam, missing
tab ohq880 recentexam, missing
...
tab svywave huq060, missing
tab svywave huq061, missing

tab svywave huq050, missing
tab svywave huq051, missing

gen hc_visit1=.
replace hc_visit1=0 if huq050==0 | huq051==0
replace hc_visit1=1 if huq050>0 & huq050<77
replace hc_visit1=1 if huq051>0 & huq051<77

tab svywave huq050, missing
tab svywave huq051, missing

gen hc_lastyr=.
replace hc_lastyr=0 if huq050==0 | huq051==0
replace hc_lastyr=0 if huq060>2 & huq060<=6 & hc_lastyr==.
replace hc_lastyr=0 if huq061>2 & huq061<=6 & hc_lastyr==.
replace hc_lastyr=1 if (huq050>0 & huq050<77) | (huq051>0 & huq051<77) | huq060==1 | huq060==2 | huq061==1 | huq061==2

gen routcare=.
replace routcare=0 if huq030==2 
replace routcare=1 if huq030==1 | huq030==3


tab hc_visit1 svywave, missing


***huq030 huq040 huq050 huq060 huq041 huq051 huq061

keep svywave riagendr ridageyr ridreth1 ridreth3 dmdeduc2 dmdmartl ridexprg wtint2yr sdmvpsu sdmvstra indhhin2 indfmin2 indfmpir dmdhhsiz dmdfmsiz dmdhhsza dmdhhszb dmdhhsze alq130 alq151 hiq011 hiq031a hiq031b hiq031d hiq031aa hiq210 hsd010 ohq845 ohdexsts ohdrcsts oharec oharocdt orxgh orxgl orxhpv smq020 smq040 talkoc currentexam recentexam doctorexam npexam dentistexam dhexam highexam lowexam examoc_n examoc_t ohq030 ohq895 ohq880 hc_visit1 huq060 huq061 routcare hc_lastyr
...
keep svywave wtint2yr sdmvpsu sdmvstra talkoc  examoc_t examoc_n currentexam recentexam doctorexam npexam dentistexam dhexam highexam lowexam male age hispanic white black asian otherrace nohs hsonly somecol college married fpl_pct childless binge insured private medicare medicaid uninsure_yr genhealth goodhealth poorhealth hpv_pos pastsmoker currentsmoker dentalvisit_1 dentalvisit_2 dentalvisit_3 dentalvisit_5 dentalvisit_never recent_dent hc_visit1 huq060 huq061 routcare hc_lastyr

save "C:\Users\jsemprini\OneDrive - University of Iowa\3-Preliminary\7-NHANES\final\oo_final\data\2011-2018nhanes_toanalyze.dta", replace
